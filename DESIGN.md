...
両方とも `OptimizerExtension::pre_optimize_function` を登録するが、DuckDB は複数登録を許容する。発火順は登録順。
duckorch は plan 解析する際、duck_lineage が同じ plan を見ていることを意識する必要はない(両者は読むだけで plan を変更しない場合は安全)。

ただし**両者とも Sentinel で plan を wrap する**ので、Sentinel が二重に被さる可能性がある。テストで確認:
- duckorch を先にロード → duck_lineage の Sentinel が duckorch Sentinel を内包
- 逆 → 逆順

どちらでも問題なく動くべき。テストケース化する。
...
### Phase 2: optimizer フック + 自動リネージ (1週)
- [ ] `OptimizerExtension::pre_optimize_function` 登録
- [ ] LogicalGet/Insert/CreateTable から I/O 抽出 (C++、duck_lineage 流用)
- [ ] LogicalOrchSentinel / PhysicalOrchSentinel 実装
...
| プロジェクト | 場所 | 何を参考にするか |
|---|---|---|
| duck_lineage | `oss-study/duck_lineage` | optimizer フック、Sentinel パターン、HTTP worker thread、OpenLineage 互換イベント生成 |
| ducksmiles | `duckdbex/duckSmiles` | C++ + Rust ハイブリッド構成、CMake / Makefile / vcpkg、FFI 設計 |
| SQLMesh | https://sqlmesh.readthedocs.io | タスクファイルフォーマット (`MODEL(...)` ヘッダ)、CLI 設計、インクリメンタルモデル |
| dbt | https://docs.getdbt.com | テスト形式、タグ、failed downstream skip、`--full-refresh` |
| Marquez | https://marquezproject.ai | OpenLineage 受け側、Mermaid との比較対象 |
| DuckDB extension-template-c | https://github.com/duckdb/extension-template-c | C API のお作法 |
| duckdb PR #16115 | https://github.com/duckdb/duckdb/pull/16115 | pre-optimization hook の仕様 |
