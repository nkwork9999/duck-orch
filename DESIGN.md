...
└── test/
    ├── unit/                          ← Rust unit
    └── e2e/                           ← Python pytest または shell ベース
```
...
OpenLineage 仕様 (https://openlineage.io/spec) に従ってイベントを emit する。
...
fn emit_event(event: OLEvent) {
    if let Some(url) = config.openlineage_url() {
        // 非同期 worker thread に enqueue(HTTP 送信はブロックしない)
        QUEUE.send(event);
    }
}
```

### 他 OpenLineage extension との連携モード

他の OpenLineage emitter 拡張がロードされている場合、duckOrch は親 pipeline run の facet を環境変数 `OPENLINEAGE_PARENT_RUN_ID` 経由で公開する。これにより同じバックエンドで親子関係付きで観測できる。
...
2. **複数の OpenLineage emitter 同時ロード**: duckOrch は pipeline 単位の親 run を発行、他拡張がクエリ単位を発行する想定。OpenLineage の `parent` facet で親子関係を保つ

### 衝突回避

他の OpenLineage emitter 拡張と一緒にロードして使う場合、両者が独立した OpenLineage イベントを emit する。
duckOrch は pipeline_run_id を環境変数で渡し、他側でそのクエリを「duckOrch run の子」として記録する想定。
...
> 実装パターン(Sentinel + デストラクタで finalize)はソフトウェア工学の一般的手法。
> 自前実装で書き起こす。
...
- [ ] OL イベント生成
- [ ] 他の OpenLineage emitter 拡張との共存テスト
...
- [ ] `inputs` 自動抽出で view を深堀りするか?(`Connection::Prepare()` で binder 通す案)
...
| [sqlglot lineage](https://github.com/tobymao/sqlglot) | MIT | column-level lineage の Scope-based アルゴリズム発想元(コードコピーは無し、AST 巡回の組み立て方のみ参考) |
